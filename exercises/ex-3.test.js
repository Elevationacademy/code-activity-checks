...
describe('Exercise 3', () => {
  ...
  it(`Should display a h1 tag with 'The Team:' as innerHTML after clicking on the about button (span)`, async function (done) {
    try {
      ...
      expect(
        header.length,
        `Could not find a h1 tag after clicking the about button (span). Use the on() method for dynamic event listener`
      ).toBe(1);
      ...
      expect(
        headerHTML,
        `Could not find a h1 tag with 'The Team:' as innerHTML after clicking the about button (span), instead we got - ${headerHTML}. Make sure you use the 'aboutHeader' variable and use the on() method for dynamic event listener`
      ).toBe('The Team:');
      ...
      expect(
        header.length,
        `Could not find one h1 tag after clicking the about button (span) twice, instead found ${header.length}. Make sure to empty the container element before appending it with the h1 tag`
      ).toBe(1);
      done();
    } catch (error) {
