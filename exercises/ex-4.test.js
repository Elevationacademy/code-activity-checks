...
describe('exercise4', () => {
  ...
  it('Player should have 2 td tags inside the tr tag: the first one with the name attribute and second with the score attribute (both passed from LeaderBoard with props)', () => {
    const p = { id: 2, name: 'Danny', score: '101' };
    const wrapper = shallow(<Player key={p.id} name={p.name} score={p.score} />);
    expect(
      wrapper.props().children[0],
      `When passing name attribute with the value of 'Danny', the first child tag should be - <td>Danny</td>. Make sure that the first td tag inside tr tag is the player's name, and that you're passing the name property from LeaderBoard to Player, using props`
    ).toEqual(<td>Danny</td>);
    expect(
      wrapper.props().children[1],
      `When passing score attribute with the value of 101, the second child tag should be - <td>101</td>. Make sure that the second td tag inside tr tag is the player's score, and that you're passing the score property from LeaderBoard to Player, using props`
    ).toEqual(<td>101</td>);
  });
});
