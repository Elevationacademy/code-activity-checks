...
describe('exercise6', () => {
  ...
  it(`Your '/reservations/total' 'get' route that receives two parameters, 'month' and 'year' should also be able to receive an optional parameter 'day'. When date has a value the route should respond with an object with a key 'total' and the value should be the total amount of reservations for the day of the month (and year) it received.`, async done => {
    if (hasError) {
      ...
      let response = await Client.get(`reservations/total/${month}/${year}?day=${day}`)
